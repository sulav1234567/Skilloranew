...
let CreateReservation = async (req, res) => {
  ...
  if (
    ...
    !status || !paymentStatEnum.includes(status.trim())||
    !method || !paymentMethodEnum.includes(method.trim())||
    !source || !sourceEnum.includes(source.trim())||
  ...
  let nooofNights = (new Date(checkoutdate) - new Date(checkindate)) / (1000 * 60 * 60 * 24);
  let totalPax = Number(adults) + Number(children);
  let isBeforeDate = new Date() - new Date(checkoutdate) <=0 && new Date() - new Date(checkindate)<0;
  if(!isBeforeDate){
    return res.status(400).json({
...
if(rooms.some((roomId) =>
  OverlappingRoomsIds.some(
    (overlappingId) =>
      overlappingId.toString() === roomId.toString()
  )
)){
return res.status(400).json({
...
let totalPayment =(Rooms.reduce((total,room)=>{
  return total + Number(room.effectivePrice) || 0
},0) * nooofNights) + Number(reservationfee)
